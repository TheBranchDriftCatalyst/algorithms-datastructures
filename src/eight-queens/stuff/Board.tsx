...
import { useFrameSVGAssemblingAnimation, FrameSVGOctagon, Text, Animator, FrameSVG, FrameSVGPathGeneric } from '@arwes/react';
...
export const Board = ({size = 8}: BoardProps): ReactElement => {
  ...
  const _solver: RecNQueensBoard = new RecNQueensBoard(size).initHooks({
  ...
  const svgRef = useRef<SVGSVGElement | null>(null);
  const { onRender } = useFrameSVGAssemblingAnimation(svgRef);
  const [solver, setSolver] = useState<RecNQueensBoard | null>(_solver);
