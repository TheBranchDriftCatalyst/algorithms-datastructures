...
def reportResults(workerReports, jobReport, timeDelta, numPortsScanned):
    if (workerReports == ''):
        # print jobReport.replace('&TIME', '%d days %d hours %d minutes %d
        # seconds' % prntime(timeDelta))
        return
    print('------------------------------------------------------------------')
...
def portScan(tgtHost, tgtPorts, quiet):
    ...
    try:
        ...
        jobReport += f"[-] Cannot resolve '{tgtHost}' Unknown host"
        return
    try:
        tgtName = gethostbyaddr(tgtIP)
        jobReport += f'[+] Scan Results for: {tgtName[0]}'
    except Exception:
        jobReport += f'[+] Scan Results for: {tgtIP}'
    finally:
    ...
    for tgtPort in tgtPorts:
        workers.append(p.apply_async(
            connScan, args=((tgtHost, int(tgtPort), quiet))))
        numPortsScanned += 1
...
def main():
    ...
    if (tgtPorts[0] == 'tcp'):
        tgtPorts: Generator[int, None, None] = iter(range(1, 65536))
    else:
        if not ((re.search('\\d+[-]\\d+', tgtPorts[0]))):
            return
        (min, max) = tgtPorts[0].split('-')
        tgtPorts = iter(range(int(min), int(max) + 1))
